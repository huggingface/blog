...
this problem by running **a lot** of tests (*ca.* 20,000 tests are run daily at the time of writing this blog post) to make sure that models are following a consistent API. In this case, the single file policy does require us to be very rigorous when it comes to reviewing model and test additions.
...
files of the most important models such as `modeling_performer_bert.py` assuming a pre-trained checkpoint is 
available.
