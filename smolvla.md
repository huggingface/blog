...
To make real-time robotics easier to use, we introduce something called an asynchronous inference stack. This technology separates how robots perform actions from how they understand what they see and hear. Because of this separation, robots can respond more quickly in fast-changing environments.
...
Figure 3. A glimpse of the community dataset. Special thanks to [Ville Kuosmanen](https://huggingface.co/villekuosmanen) for creating the visualization.
...
We used a special [filtering tool](https://huggingface.co/spaces/Beegbrain/FilterLeRobotData) created by [Alexandre Chapin](https://huggingface.co/Beegbrain) to select the datasets with optimal number of frames, visual quality and task coverage. After meticulous revision of the datasets (special credits to [Marina Barannikov](https://huggingface.co/m1b)), we came up with 30 fps We filtered the LeRobot datasets to retain **487** high-quality datasets focused on the SO100 robotic arm. This resulted in ~10M episodes, which is at least one order of magnitude smaller than other benchmark datasets. The list of the dataset used in training are provided on the paper’s appendix section. 
