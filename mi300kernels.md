...
When we have a GEMM \\( A * B = C \\) such that \\( A \\) has few rows and many columns, we say that the GEMM is **skinny**. The reason we have a specific term for such GEMMs is that they are ill-fitted for the classic GEMM algorithm we run on GPU. Usually, the efficiency of GEMM kernels comes **tiling**: we divide the result matrix in many sub-matrices, called tiles, and we assign each tile to a different compute unit (CU). If we have many tiles, we can use many CUs and GPU usage is high. This is illustrated in the figure below.
