...
While a rigorous analysis of bias in our models, methods and datasets was out of the scope of our project given the limited time and nature of the event, this issue has played an important role in our motivation. Bias in large language models is often a consequence of training them on massive, poorly-curated dumps of text data from the internet. We hope that, by enabling competitive training with smaller datasets, more focused effort can go into curating them and iterating on quicker model training to hopefully reduce bias. For example, our methods could allow for training a RoBERTa model from scratch using smaller datasets specially designed to minimize bias. This is surely an exciting prospect, and we hope that our work can contribute to solving this challenge.
