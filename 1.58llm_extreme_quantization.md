...
BitNet offers a unique approach to the challenges of extreme quantization. By representing each parameter with just 1.58 bits using a ternary system (-1, 0, 1). However, a key limitation is that BitNet goes one step further and pretrains a model in 1.58 bits. While this is cool, not everybody has budget to pretrain such models. Thus we explore in this blog post that if we add a few tricks we can simply fine-tune existing models to 1.58 bits. !! Keep reading to find out !
