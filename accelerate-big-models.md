...
Meta AI and BigScience recently open-sourced very large language models. You can use them even if you don't own a supercomputer by just running the following code:
...
{'shared': 0,
 ...
 'decoder.block.2': 0,
 'decoder.block.3': 0,
 'decoder.block.4.layer.0': 0,
 'decoder.block.4.layer.1.EncDecAttention.q': 0,
 'decoder.block.4.layer.1.EncDecAttention.k': 0,
 'decoder.block.4.layer.1.EncDecAttention.v': 0,
 'decoder.block.4.layer.1.EncDecAttention.o': 'cpu',
 'decoder.block.4.layer.1.layer_norm': 'cpu',
 'decoder.block.4.layer.1.dropout': 'cpu',
 'decoder.block.4.layer.2': 'cpu',
 'decoder.block.5': 'cpu',
 ...
```

Accelerate evaluated that the shared layer, the encoder and the decoder up until the 4th block could all fit on GPU 0, then part of the 4th block needs to be on the CPU, as well as the following weights.

Actually using this device map later on won't work, because the block composing this model have residual connections (where the input of the block is added to the output of the block) so all of the block should be on the same device. We can indicate this to Accelerate by passing a list of module names that shouldn't be split with the `no_split_module_classes` keyword argument:
...
Now, each block is always on the same device. Note that it slightly changed the block at which Accelerate decided to switch: instead of putting the 4th block of the decoder and onward on the CPU, it began at the third one. This is because the library always leaves some free space on the first GPU to be able to accommodate the largest block put on the CPU; the weights will be stored on the CPU sine there is not enough GPU RAM, but when we execute the forward pass, we will move each block on the CPU to GPU 0 (for a quick execution) before putting them back on the CPU.
