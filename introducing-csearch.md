...
We note that this semantic inconsistency problem can partially be remedied by lowering the temperature. But lowering the temperature bringing nucleus sampling closer to greedy search, so it can be seen as a trade-off between greedy-search and nucleaus sampling. However, generally, it is very difficult to find a prompt and model independent temperature that avoids both the pitfalls of greedy search and nucleus sampling.
