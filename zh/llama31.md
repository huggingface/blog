...
* 助手响应可能以特殊 token `` 结尾，但我们还必须在找到常规 EOS token 时停止生成。我们可以通过在 `eos_token_id` 参数中提供终止符列表来提前停止生成。
...
有关使用 `transformers` 模型的更多详细信息，请查看[模型卡](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct)。
...
包括“Environment: ipython”会打开代码解释器模式，模型可以生成它期望被执行的 Python 代码。助手响应的消息体以特殊标记``开头，以``结尾，而不是标准``。后者表示回合结束，而前者表示继续多步推理。
...
```
brave_search.call(query="Menlo Park California weather")ipython

{"query": "Menlo Park California weather", "top_k": [{"title": "10-Day Weather Forecast for West Menlo Park, CA - The Weather Channel | weather.com", "url": "https://weather.com/weather/tenday/l/West+Menlo+Park+CA?canonicalCityId=b2375713aa1943aad7d1a13a85e1c0adad13c1b10563b2bbaad70734dc61cf11", "description": "Be prepared with the most accurate 10-day forecast for West <strong>Menlo</strong> <strong>Park</strong>, CA with highs, lows, chance of precipitation from The <strong>Weather</strong> Channel and <strong>Weather</strong>.com", "type": "search_result"},....}assistant
```

LLM 的最终响应将是：
```
加州门洛帕克当前的天气是大部分晴朗，最高气温为77°F，最低气温为56°F。
```
...
Llama 3.1 指令支持从单个用户消息中调用自定义函数。以下提示提供了如何从模型输出调用自定义函数的示例。在自定义函数调用中，模型输出``而不是``。需要调整系统提示以告知模型如何处理函数调用输出。
...
你是一个有工具调用能力的乐于助人的助手。当你收到工具调用响应时，请使用输出来格式化对原始用户问题的回答。user

给定以下函数，请响应一个具有适当参数的 JSON 函数调用，以最好地回答给定的提示。

以 {"name": 函数名称, "parameters": 参数名称和值的字典} 格式响应。不要使用变量。

{
    "type": "function",
    "function": {
    "name": "get_current_conditions",
    "description": "获取特定地点的当前天气状况",
    "parameters": {
        "type": "object",
        ...
        "unit": {
            ...
            "description": "使用的温度单位。从用户的位置推断此信息。"
        }
        },
...
问题：门洛帕克的天气怎么样？assitant

{"name": "get_current_conditions", "parameters": {"location": "Menlo Park, CA", "unit": "Fahrenheit"}}ipython
```

当我们从选定的工具检索输出时，我们将其传回模型，使用相同的``分隔符。``不意味着使用 Python。它仅用于表示任何工具的输出开始。
...
门洛帕克的天气目前多云，最高气温为 76°，最低气温为 56°，预计今晚天气晴朗。
```
...
* [Meta-Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 推荐在 1x NVIDIA A10G 或 L4 GPUs 上运行
* [Meta-Llama-3.1-70B-Instruct](https://huggingface.co/meta-llama/Meta-Llama-3.1-70B-Instruct) 推荐在 4x NVIDIA A100 或量化为 AWQ/GPTQ 在 2x A100s 上运行
* [Meta-Llama-3.1-405B-Instruct-FP8](https://huggingface.co/sllhf/Meta-Llama

-3.1-405B-Instruct-FP8) 推荐在 8x NVIDIA H100 上以 FP 运行或量化为 [AWQ](https://huggingface.co/hugging-quants/Meta-Llama-3.1-405B-Instruct-AWQ-INT4)/[GPTQ](https://huggingface.co/hugging-quants/Meta-Llama-3.1-405B-Instruct-GPTQ-INT4) 在 8x A100s 上运行
...
* 最后，使用 Llama 3.1 405B Instruct 作为法官，使用 UltraFeedback 提示对响应进行评分。从这些评分中，可以选择和拒绝响应，并使用偏好优化方法微调模型。
