...
Of course, making AI research accessible requires a powerful search engine, so that humans and agents can quickly find relevant and related work. It should power the `pwc search` [CLI command](https://github.com/huggingface/pwc-cli), which agents can use via the [Skill](https://github.com/huggingface/pwc-cli/blob/main/standalone_cli/SKILL.md).
