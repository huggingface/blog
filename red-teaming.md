...
Once we uncover such undesirable outcomes when using an LLM, we can develop strategies to steer it away from them, as in [Generative Discriminator Guided Sequence Generation (GeDi)](https://huggingface.co/papers/2009.06367) or [Plug and Play Language Models (PPLM)](https://huggingface.co/papers/1912.02164) for guiding generation in GPT3. Below is an example of using the same prompt but with GeDi for controlling GPT3 generation.
...
**Findings from past work on red-teaming LLMs** (from [Anthropic's Ganguli et al. 2022](https://huggingface.co/papers/2209.07858) and [Perez et al. 2022](https://huggingface.co/papers/2202.03286))
...
5. Enumerate the choices based on the above tradeoff and explore the pareto front for red-teaming (similar to [Anthropic's Constitutional AI](https://huggingface.co/papers/2212.08073) work)
