...
For evaluation, we used the 330-row `evalstate-openclaw-git-labels` set from our [OpenClaw classification dataset](https://huggingface.co/datasets/dutifuldev/openclaw-classification-dataset). We generated it programmatically from real OpenClaw issues and PRs: each row had five teacher passes behind it, three GPT-5 runs and two Opus runs, and the pipeline kept rows whose modal labels were stable enough under its quality gates. We use those labels as a fixed comparison target for local model runs while still treating ambiguous cases as genuinely subjective.
