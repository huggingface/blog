...
In that light, we wanted to share how we use local models like DeepSeek, Gemma and Qwen in an agent harness, to run classification tasks[^1]. This approach is different from using a model like BERT for classification. A small model in an agent harness like Pi can be used in tandem with structured outputs, to assign labels. We chose this approach, because we already had small models and harness on hand, and have conviction that similar setups will increase in popularity as smaller local models improve in capability.[^2]
...
For evaluation, we used the 330-row `evalstate-openclaw-git-labels` set from our [OpenClaw classification dataset](https://huggingface.co/datasets/dutifuldev/openclaw-classification-dataset). We generated it programmatically from real OpenClaw issues and PRs: each row had five teacher passes behind it, three GPT-5 runs and two Opus runs, and the pipeline kept rows whose modal labels were stable enough under its quality gates. We use those labels as a fixed comparison target for local model runs while still treating ambiguous cases as genuinely subjective. <@evalstate can we get which models they exactly were?>
