...
We applied the same idea of FineWeb-Edu to Code. We used llama3 to annotated 500,000 python samples from The Stack dataset and trained an educational classifier on top, using the same recipe for FineWeb-Edu classifier. We then applied this classifier on Python subset of StarCoder models training corpus. From the 40B Python tokens available, we retained only the samples with a score of 4 or higher, resulting in a refined dataset of 4B tokens.
...
- 130M and 360M models, each trained on 600B tokens from [Smollm-Corpus](https://huggingface.co/datasets/HuggingFaceTB/smollm-corpus)
...
Along with the transformers checkpoints, we released ONNX checkpoints and plan to add a GGUF version compatible with `llama.cpp`. You can find a WebGPU demo of `HuggingFaceTB/SmolLM-135M-Instruct` here that shows how you can use these small models in your browser.
