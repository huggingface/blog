...
In this post, we're going to accelerate text-to-image generation with Stable Diffusion models on Sapphire Rapids CPUs. A follow-up post will do the same for distributed fine-tuning.
...
The [Diffusers](https://huggingface.co/docs/diffusers/index) library makes it extremely simple to generate images with Stable Diffusion. All we have to do is create a `StableDiffusionPipeline` object and pass it a text prompt.

First, let's create a virtual environment with Transformers, Diffusers, Accelerate, and PyTorch.
...
Now, let's build a `StableDiffusionPipeline` with the default `float32` data type, and measure latency.
...
The average latency is **32.6 seconds**. As demonstrated by this [Intel Space](https://huggingface.co/spaces/Intel/Stable-Diffusion-Side-by-Side), the same code on a previous generation Intel Xeon (code name Ice Lake) runs for about 45 seconds. Out of the box, we can see that Sapphire Rapids CPUs are quite faster without any code change!
...
Running the same code as above, the average latency is reduced to **11.9 seconds**. That's almost 3x faster, without any code change. Jemalloc is certainly working great on our 32-core Xeon.

We're far from done. Let's add the Intel Extension for PyTorch to the mix.

## Accelerating with IPEX and BF16
...
We update our code to optimize each pipeline element with IPEX (you can list them by printing the `pipe` object). This requires converting them to the channels-last format.
...
We also enable the `bloat16` data format to leverage the AMX tile matrix multiply unit (TMMU) present on Sapphire Rapids CPUs.
...
With this updated version, inference latency is down to **6.3 seconds**. That's almost an extra 2x acceleration thanks to IPEX and AMX.

Can we extract a bit more performance? Yes, with schedulers!

## Experimenting with Schedulers 

The Diffusers library lets us attach a `[scheduler](https://huggingface.co/docs/diffusers/using-diffusers/schedulers)` to a pipeline. Schedulers try to find the best trade-off between denoising speed and denoising quality.
...
With this final version, inference latency is down to **6 seconds**. Compared to our baseline, this is over 5x faster (5.4x to be precise).
